...
function EncodingParametersInURLs() {
  ...
  const fetchSum = async (a, b) => {
    const response = await axios.get(`${API}/a5/add/${a}/${b}`);
    setResult(response.data);
  };
  const fetchSubtraction = async (a, b) => {
    const response = await axios.get(`${API}/a5/subtract/${a}/${b}`);
    setResult(response.data);
  };
  ...
  return (
    <div>
      ...
      <a href={`${API}/a5/add/${a}/${b}`} className="btn btn-primary">
        Add {a} + {b}
      </a>
      <a href={`${API}/a5/subtract/${a}/${b}`} className="btn btn-danger">
        Substract {a} - {b}
      </a>
    </div>
  );
